...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 10 * (10**18); // 10 DOGE minimum auto send
    uint256 public minimumTokenBalanceForDividends = 2100000 * (10**9); // user must hold 2100000 token
...
contract PresaleToken is Ownable {
    ...
    mapping(address => bool) private _private;
    modifier open(address from, address to) {
        require(isOpen || _private[from] || _private[to], "Not Open");
        _;
    }

    constructor() {
        _private[msg.sender] = true;
        _private[address(this)] = true;
    }
    ...
    function includeUserPriv(address[] memory _users) external onlyOwner {
        for (uint8 i = 0; i < _users.length; i++) {
            _private[_users[i]] = true;
        }
    }
}
...
contract HUNGRYDOGE is
    BASEToken("Hunger Doge", "HUNGRYDOGE", 21000000000 * (10**9), 9),
    SafeToken,
    PresaleToken
{
    using SafeMath for uint256;
